...
const VideoProvider = ({children}) => {
    ...
    useEffect(
        () => ( async () => {
            try{
                const { data: {videos}, status } = await getVidoes();
                if(status === 200)
                    videoListDispatch({type:"LOAD_VIDEOS", payload: videos});
                throw new Error("Falied with code: ", status);
            }catch{
...
const useVideoProvider = () => {
    ...
    if(context === undefined)
        throw new Error("Context must be defined first.");
    return context;
}
